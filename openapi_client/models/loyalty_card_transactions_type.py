# coding: utf-8

"""
    Voucherify API

    Voucherify promotion engine REST API. Please see https://docs.voucherify.io/docs for more details.

    The version of the OpenAPI document: v2018-08-01
    Contact: support@voucherify.io
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import json
import pprint
import re  # noqa: F401
from aenum import Enum, no_arg





class LoyaltyCardTransactionsType(str, Enum):
    """
    LoyaltyCardTransactionsType
    """

    """
    allowed enum values
    """
    POINTS_ACCRUAL = 'POINTS_ACCRUAL'
    POINTS_CANCELLATION = 'POINTS_CANCELLATION'
    POINTS_REDEMPTION = 'POINTS_REDEMPTION'
    POINTS_REFUND = 'POINTS_REFUND'
    POINTS_ADDITION = 'POINTS_ADDITION'
    POINTS_REMOVAL = 'POINTS_REMOVAL'
    POINTS_EXPIRATION = 'POINTS_EXPIRATION'
    POINTS_TRANSFER_IN = 'POINTS_TRANSFER_IN'
    POINTS_TRANSFER_OUT = 'POINTS_TRANSFER_OUT'

    @classmethod
    def from_json(cls, json_str: str) -> LoyaltyCardTransactionsType:
        """Create an instance of LoyaltyCardTransactionsType from a JSON string"""
        return LoyaltyCardTransactionsType(json.loads(json_str))


